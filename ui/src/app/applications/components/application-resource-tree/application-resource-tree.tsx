        ...
        <React.Fragment>
            <div className='application-resource-tree__node' style={{left: node.x, top: node.y, width: node.width, height: node.height}}>
                ...
                <div className='application-resource-tree__node--expansion' onClick={(event) => { expandCollapse(node, props); event.stopPropagation();}}>
                    {props.getNodeExpansion(node.kind +":" + node.name) ? (
...
function renderResourceNode(props: ApplicationResourceTreeProps, id: string, node: ResourceTreeNode & dagre.Node, nodesHavingChildren: Map<string, number>) {
    ...
    // let children = findNetworkTargets(networkNodes, node.networkingInfo, props);


    // nodes.forEach(aNode => {
    //         aNode?.parentRefs?.forEach(parentNode => {                    
    //             // const children = childrenByParentKey.get(treeNodeKey(parentNode)) || [];
    //             if (parentNode.kind == node.kind && parentNode.name == node.name) {
    //                 if (children.length == 0) {
    //                     children = childrenByParentKey.get(treeNodeKey(parentNode)) || [];
    //                 }
    //             }
    //             if (node.kind === "Deployment") {
    //                 console.log("node   " + node.kind + " , " + node.name + ", " + node.uid);
    //                 console.log("parent " + parentNode.kind + " , " + parentNode.name + ", " + parentNode.uid);
    //                 console.log(node.name + ", " + node.kind + ", CHILDREN = " + children.length);
    //             }
    ...
    const childCount = nodesHavingChildren.get(node.kind + ":" + node.name);
    // console.log(node.name + ", " + node.kind + ", CHILDREN = " + childCount);
...
export const ApplicationResourceTree = (props: ApplicationResourceTreeProps) => {
    ...
    if (props.useNetworkingHierarchy) {
        ...
        networkNodes.forEach(parent => {
            findNetworkTargets(networkNodes, parent.networkingInfo, props).forEach(child => {
                ...
                if (nodesHavingChildren.has(parent.kind + ":" + parent.name)) {
                    let c = nodesHavingChildren.get(parent.kind + ":" + parent.name);
                    nodesHavingChildren.set(parent.kind + ":" + parent.name, c + children.length);
                } else {
                    nodesHavingChildren.set(parent.kind + ":" + parent.name, 1);
                }
    ...
    graph.edges().forEach(edgeInfo => {
        ...
        if (edge.points.length > 1) {
            ...
            // }
            // const nPoints = edge.points.length-1;
            ...
            //     offset = c;
            // } else {
            //     offset = nodeOffset.get(edgeInfo.v);
            // }
            ...
            if (edgeInfo.v === EXTERNAL_TRAFFIC_NODE) {
                ...
                // let yEnd = endNode.y + 10*offset;
                let yEnd = endNode.y + (offset * 10) * (offset%2 != 0 ? 1 : -1);
                if (yEnd > (endNode.y + endNode.height/2)) {
                    yEnd = endNode.y + endNode.height/2;
                } else if (yEnd < (endNode.y - endNode.height/2)) {
                    yEnd = endNode.y - endNode.height/2;
                }
                let av = reverseEdge.get(edgeInfo.w);
                let len = av + 1;
                yEnd = endNode.y - endNode.height/2 + (endNode.height/len + (endNode.height/len * offset));

                const startNodeRight = 142;
                // let firstBend = startNode.x + 180 + 10*offset;
                let firstBend = startNode.x + startNodeRight+ (endNode.x - startNode.x - 140-142) / len + ((endNode.x - startNode.x - 140- 142) / len * offset); 
                ...
                lines.push({x1: firstBend, y1: yEnd, x2: endNode.x-140, y2: yEnd});
                // lines.push({x1: edge.points[0].x+65*2, y1: edge.points[0].y, x2: edge.points[nPoints].x-70, y2: edge.points[0].y});
                // lines.push({x1: edge.points[nPoints].x-70, y1: edge.points[0].y, x2: edge.points[nPoints].x-70, y2: edge.points[nPoints].y});
                // lines.push({x1: edge.points[nPoints].x-70, y1: edge.points[nPoints].y, x2: edge.points[nPoints].x, y2: edge.points[nPoints].y});    
            }
        }
